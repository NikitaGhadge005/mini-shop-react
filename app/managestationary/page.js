...
export default function ProductTable() {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      const res = await fetch(`/api/stationary?id=${id}`, {
        method: 'DELETE',
      });
      ...
      <div className="flex-1 p-6 overflow-y-auto">
          ...
          <table className="min-w-full text-sm text-gray-700">
            ...
            <tbody>
              {products.map((p, index) => (
                <tr key={p._id || index} className="border-t hover:bg-gray-50 transition">
                  <td className="p-4">
                    <img
        ...
        {showModal && selectedProduct && (
          <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="bg-white w-full max-w-md max-h-screen overflow-y-auto mx-4 md:mx-auto rounded shadow-lg p-6 relative">
              {/* Close Button */}
              <button
